...
<body>
        ...
        <tbody>
            <tr>
                <td width="45%" class="border" style="vertical-align: top">
                    ...
                    top: 0;
                    right: 0;
                    padding: 20px; font-weight:bold font-family: 'Times New Roman', Times, serif; font-size: 10pt;font-style:italic">{{$hoadon[0]->mahoadon}}</div>
                </td width="55%">
            </tr>
        </tbody>
    </table>
        ...
        <tbody>
            <tr>
                <td colspan="3" class=" border">
                    <div> Họ và tên sinh viên:&nbsp; <strong>{{$hoadon[0]->hoten_sv}}</strong></div>
                </td>
            </tr>
            <tr>
                <td colspan="3" class=" border">
    ...
    <table class="table table-bordered table-hover table-striped" style="width: 100%;margin-top: 10px">
        <thead>
            <tr>
                ...
                <th style="border-bottom: 1px solid black;text-align:center">Nhà sản xuất</th>
        ...
        <tbody>
            @for ($i = 0; $i < count($hoadon); $i++)
                @if ($i == count($hoadon) - 1)
                <tr>
                    <td  style="border-bottom: 1px solid black;text-align:center">{{$hoadon[$i]->loai}}</td>
                    <td  style="border-bottom: 1px solid black;text-align:center">{{$hoadon[$i]->size}}</td>
                    <td  style="border-bottom: 1px solid black;text-align:center">{{$hoadon[$i]->nsx}}</td>
                    <td  style="border-bottom: 1px solid black;text-align:center">{{$hoadon[$i]->sl_phat}}</td>
                </tr>
                @else
                <tr>
                    <td style="text-align:center">{{$hoadon[$i]->loai}}</td>
                    <td style="text-align:center">{{$hoadon[$i]->size}}</td>
                    <td style="text-align:center">{{$hoadon[$i]->nsx}}</td>
                    <td style="text-align:center">{{$hoadon[$i]->sl_phat}}</td>
                </tr>
                @endif
    ...
    <table style="margin-top:30px; ">
        <tbody>
            ...
            <tr>
                <td class=" center border" width="50%" height = "120px" style="vertical-align: center; padding-right:100px;padding-top:80px;">
                    <div><strong>{{$hoadon[0]->hoten_sv}}</strong></div>
                </td>
                <td  class=" center border " width="50%" height = "120px" style="vertical-align: center;padding-top:80px;">
                    <div><strong>{{$hoadon[0]->hoten_nguoiphat}}</strong></div>
                </td>
            </tr>
        </tbody>
    </table>
