                    ...
                    <div class="row">
                        <div class="col-12 col-xl-8 col-md-12">
                            <fieldset class="card card-body">
                                ...
                                <div style="min-height: 600px;" class="direct-chat-messages">
                                    @if (count($history) > 0)
                                        @foreach ($history as $ls)
                                            @if ($ls->id_nhansu == 0)
                                                <div class="direct-chat-msg">
                                                    <div class="direct-chat-infos clearfix">
                                                        @if(empty($name_user->hoten ))
                                                            <span class="direct-chat-name float-left"></span>
                                                            <span class="direct-chat-timestamp float-right"></span>
                                                        @else
                                                            <span class="direct-chat-name float-left">{{ $name_user->hoten }}</span>
                                                            <span class="direct-chat-timestamp float-right">{{ $ls->create_at }}</span>
                                                        @endif
                                                    </div>
                                                    <img class="direct-chat-img" src="{{Auth::guard('loginbygoogles')->user()->img_gg}}"
                                            ...
                                            @else
                                                <div class="direct-chat-msg right">
                                                    <div class="direct-chat-infos clearfix">
                                                        <span
                                                            class="direct-chat-name float-right">CTUT-Tuyển sinh</span>
                                                        <span
                                                            class="direct-chat-timestamp float-left">{{ $ls->create_at }}</span>
                                                    </div>
                                                    <img class="direct-chat-img" src="/img/CTUT_logo.png"
                                                        alt="message user image">
