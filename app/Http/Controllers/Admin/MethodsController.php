...
class MethodsController extends Controller
{
    ...
    public function create(){
        return view('admin.methods.methods');
    }
    ...
    public function loadMethods(){
        $methods = DB::table('l_method')->orderBy('id', 'asc')->get();
        $json_data['data'] =  $methods;
        $data = json_encode($json_data);
        echo  $data;
    }

    // public function loadSchools($id){
    //     $shools = DB::table('l_school')->where('id_province',$id)->orderBy('id_school', 'asc')->get();
    //     $json_data['data'] = $shools;
    //     $data = json_encode($json_data);
    //     echo  $data;
    // }
    ...
    // $image = $request->file('file_council');
    // $path = 'images/years/2023/HDTS';
    ...
    //     // ->join('roles', 'menus.id', '=', 'roles.iduser')
    //     // // ->select('menus.*')
    //     // ->where('active', 1)
    //     // ->orWhere('iduser',10)
    //     // ->orderBy('number', 'asc')
