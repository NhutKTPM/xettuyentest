...
class TraCuuKetQuaController extends Controller
        ...
        if ($validator->fails()) {
            ...
            try{
                ...
                foreach ($datas as $key => $value) {
                    if(strlen($value[2]) > 1000){
                        $del_img = DB::table('l_image_hocba')
                        ->where('id_user',Auth::guard('user')->user()->id)
                        ->where('type_img',$value[0])
                        ->get();
                        if(count($del_img) >0 ){
                            foreach ($del_img as $key => $value1) {
                                if(File::exists(ltrim($value1 ->link_img,"/"))){
                                    unlink(ltrim($value1 ->link_img,"/"));
                                }
                            }
                            DB::table('l_image_hocba')
                            ->where('id_user',Auth::guard('user')->user()->id)
