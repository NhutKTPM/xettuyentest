...
$(document).ready(function(){
   ...
   // Load Search
    ...
    $('#excel_ex_sta').on('click',function(){
     ...
     //Xuaat exel
     $('#print_ex_sta').on('click',function(){
        var year = $('#year_ex_sta').val();
        var form = $('#form_ex_sta').val();
        var user = $('#user_ex_sta').val();
        var day1 = $('#day_ex_sta').val();
        if(user == 0){
            toastr.warning("Vui lòng chọn người thu")
        }else{
            if(day1 == ""){
                day = 0
            }else{
            ...
            var winPrint = window.open('/admin/expenses_sta/data_print/'+year+"/"+form+'/'+user+'/'+day, 'mywindow', 'left=100,top=100,width=1200,height=900,toolbar=0,scrollbars=0,status=0');
            winPrint.print()
        }
    });
});
