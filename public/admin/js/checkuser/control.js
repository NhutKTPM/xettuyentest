$(document).ready(function(){
                    ...
                    success: function (res) {
                        ...
                        switch (res) {
                            ...
                            case '2':
                                ...
                                break;
                            case '3':
                                toastr.warning('Hồ sơ đã khóa, vui lòng mở khóa hồ sơ trước khi lưu')
                                break;
                            default:
                        ...
                        check_user_load_list_file_maphieu(id_user)
                    }
                });
            }
        }
    ...
    $('#check_user_phieu_list_file').on('click',function(){
        ...
        // window.open('https://xettuyentest.ctuet.edu.vn/admin/checkuser/check_user_phieu_list_file/'+id_user,'_blank')
        check_user_his_list_file(id_user)
        check_user_load_list_file_maphieu(id_user)
    })
...
function change_check(){
    $('#modal_loadding_check_user').show()
    $('#change_check').attr('disabled','true')
    ...
    if(c_major.length > 0){
        ...
        $.ajax({
            ...
            success: function (res){
                $('#change_check').removeAttr('disabled')
                $('#modal_loadding_check_user').hide()
                check_user_load_list_file_maphieu(id_user)
                switch (res) {
                    case '1':
                    ...
                    case '5':
                        ...
                        break;
                    case '6':
                        toastr.warning("Cán bộ không được quyền cho trúng tuyển trực tiếp, vui lòng liên hệ cán bộ quản lý")
                        break;
                    default:
...
function load_view(id){
    ...
    check_user_his_list_file(id)
    check_user_load_list_file_maphieu(id)
}
                ...
                var html = '<div class = "row""><strong>Danh sách nguyện vọng đăng ký</strong>'
                    ...
                    html += '<div class="col-12 col-md-3">'
                        html += '<div class="row">'
                            ...
                            html += '<div class="col-12 col-md-6">'
                                html +='<button type="button" id="change_check" class="btn btn-block btn-primary btn-xs" onclick = "change_check()" ><i class="fa fa-check" ></i>&nbsp;&nbsp;&nbsp;Trúng tuyển</button>'
                            html += '</div>'
                        html += '</div>'
                    html += '</div>'
                    ...
                    html += '<div class="col-12 col-md-3">'
                        html += '<div class="row">'
                            html += '<div class="col-12 col-md-6">'
                                html +='<button type="button" id="change_check_file" class="btn btn-block btn-primary btn-xs" onclick = "change_check_file()" ><i class="fa fa-print" ></i>&nbsp;&nbsp;&nbsp;In GBTT</button>'
                            html += '</div>'
            ...
            if(res == 1){
                ...
                if(res == 0){
                    ...
                    if(res == 3){
                        ...
                        if(res == 4){
                            toastr.warning("Chưa thu hồ sơ thí sinh. Vui lòng thu hồ sơ thí sinh trước khi khóa")
                        }else{
                            if(res == 5){
                                toastr.warning('Thí sinh chưa xác nhận trên Cổng của Trường, Vui lòng yêu cầu thí sinh xác nhận trên cổng tra cứu và upload hình ảnh')
...
function check_user_load_list_file_maphieu(id){
    $('#check_user_load_list_file_maphieu').html('')
    $.ajax({
        type: "get",
        url: "checkuser/check_user_load_list_file_maphieu/"+id,
        success: function (res) {
            var html = ""
            html += "<span>Mã phiếu trúng tuyển:&nbsp;&nbsp;&nbsp;</span><span style ='color:#007bff;font-weight:bold'>"+res.maphieu+"</span><br>"
            html += "<span>Trạng thái trúng tuyển:&nbsp;&nbsp;&nbsp;</span><span style ='color:#007bff;font-weight:bold'>"+res.tt+"</span><br>"
            html += "<span>Xác nhận cổng Trường:&nbsp;&nbsp;&nbsp;</span><span style ='color:#007bff;font-weight:bold'>"+res.xn+"</span><br>"
            html += "<span>Xác nhận cổng Bộ:&nbsp;&nbsp;&nbsp;</span><span style ='color:#007bff;font-weight:bold'>"+res.nhaphoc+"</span></br>"
            html += "<span>Mã phiếu rà soát:&nbsp;&nbsp;&nbsp;</span><span style ='color:#007bff;font-weight:bold'>"+res.rs+"</span></br>"
            html += "<span>Hồ sơ sinh viên (P.CTCT&QLSV):&nbsp;&nbsp;&nbsp;</span><span style ='color:#007bff;font-weight:bold'>"+res.hssv+"</span></br>"
            $('#check_user_load_list_file_maphieu').html(html)
        }
    });
}
