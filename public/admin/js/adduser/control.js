$(document).ready(function(){
    ...
    $('#id_card_reset_save').on('click',function(){
        ...
        $('#id_card_reset_save').attr('disabled','true');
        var batch = $('#batch_card_reset').val();
        var cmnd = $('#id_card_reset').val()
        if(batch == 0){
            toastr.warning("Chọn đợt tuyển sinh")
            $('#modal_loadding_add').hide();
            $('#id_card_reset_save').removeAttr('disabled');
        }else{
            if(cmnd == '' || cmnd == 0){
    ...
    $('#batch_card_reset').select2()
    batch_card_reset()
})

function batch_card_reset(){
    $.ajax({
